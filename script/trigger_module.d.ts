declare module p_nm2d {
	export let 玩家数据全局: p_nm2d.玩家数据;
	/** 
	 * @name Class_of_Luatable */
	 ...
	 * @name 数据库有无值回调 */
		...
		constructor(obj:base.Game, 有无:boolean, 数据列表:LuaTable<string, string> | undefined)
	 ...
	 * @name 客户端退出游戏服务端存档数据 */
		...
		constructor(obj:base.Game, 数据列表:LuaTable<string, string>, player_slot_id:number)
		readonly event_name: string
		readonly autoForward: boolean
	}
	 ...
	 * @name 阶段存档 */
		...
		是否开启背叛: boolean
		player_slot_id: number
		constructor(obj:base.Game, 当前为第几天的存档:number, 金钱:number, 项目完成度:number, 妻子崩溃度:number, 后辈好感度:number, 是否开启背叛:boolean, player_slot_id:number)
		readonly event_name: string
		readonly autoForward: boolean
	}
